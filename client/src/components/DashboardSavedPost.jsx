...
export default function SavedPosts() {
  ...
  useEffect(() => {
    const fetchSavedPosts = async () => {
      try {
        // Fetching the posts using the correct route
        const res = await fetch(`/api/post/getposts?userId=${currentUser._id}`);
        const data = await res.json();
        if (res.ok) {
          setSavedPosts(data.posts); // Assuming 'posts' is the key returned
        }
        setLoading(false);
      } catch (error) {
          ...
          <Table.Body>
            {savedPosts.map((post) => (
              <Table.Row key={post._id} className="bg-white dark:bg-gray-800">
                <Table.Cell className='text-xs'>{new Date(post.updatedAt).toLocaleString()}</Table.Cell>
                <Table.Cell>
                  <Link to={`/post/${post.slug}`}>
                    <img
                      src={post.image}
                      alt={post.title}
                      className='w-20 h-12 rounded-md object-cover dark:bg-gray-500'
                    />
                  </Link>
                </Table.Cell>
                <Table.Cell>
                  <Link
                    to={`/post/${post.slug}`}
                    className='font-medium text-xs text-gray-600 dark:text-white'
                  >
                    {post.title}
                  </Link>
                </Table.Cell>
                ...
                <Table.Cell>
                  <Link
                    to={`/post/${post.slug}`}
                    className='text-purple-500 text-xs cursor-pointer font-medium hover:text-purple-600'
                  >
                    {post._id}
                  </Link>
                </Table.Cell>
